...
public class TestUnionWEmptyEquivalenceClass {

    @Test
    public void testUnionOperationWithEmptyEquivalenceClass() throws Exception{

        ExecutionEnvironment env = ExecutionEnvironment
                .getExecutionEnvironment();

        DataSource<String> visits = env.readTextFile(Config.pathToVisits());
        DataSource<String> visitsEmpty = env.readTextFile(Config.pathToEmptyVisits());

        DataSet<Tuple3<String, String, Integer>> visitSet = visits.flatMap(
                new VisitsReaderWithCount());


        DataSet<Tuple3<String, String, Integer>> visitEUSet = visitsEmpty.flatMap(new VisitsReaderWithCount());


        DataSet<Tuple3<String, String, Integer>> visitsUnion = visitSet.union(visitEUSet);

        visitsUnion.printOnTaskManager("UnionSet");
